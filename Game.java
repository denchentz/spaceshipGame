...
public class Game extends JPanel implements ActionListener, KeyListener {
	...
	public void draw(Graphics g) {
        ...
        if (gameOver) {
            ...
            g.drawString("Score: " + String.valueOf((int) score), 110, 330);
        }
        else {
    ...
    public void move() {
        ...
        // Check collision using Quad Tree
        List<GameObject> returnObjects = new ArrayList<>();
        returnObjects.clear();
        // Find asteriods close to the rocket 
        returnObjects = quadTree.retrieve(returnObjects, rocket);
        for (GameObject asteriod: returnObjects) {
            // Run collision detection algorithm between the rocket and asteriods
            if (collision(rocket, asteriod)) {
                int x = (rocket.x + asteriod.x) / 2;
                int y = (rocket.y + asteriod.y) / 2;
                explode = new Explode(x - 16, y - 16, asteriodImg);
                gameOver = true;
            }
        }
    ...
    @Override
    public void actionPerformed(ActionEvent e) {
        // remake the Quad Tree every frame
        quadTree.clear();
